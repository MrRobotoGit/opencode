...
export function fromConfig(permission: ConfigPermission.Info) {
  ...
  for (const [key, value] of Object.entries(permission)) {
    const normalizedPermission = ShellToolID.normalize(key)
    if (typeof value === "string") {
      ruleset.push({ permission: normalizedPermission, action: value, pattern: "*" })
      continue
    }
    ruleset.push(
      ...Object.entries(value).map(([pattern, action]) => ({ permission: normalizedPermission, pattern: expand(pattern), action })),
    )
  }
  return ruleset
}
